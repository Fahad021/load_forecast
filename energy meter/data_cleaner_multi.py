...
def get_EndTime(last_t):
    ...
    return f'{str(d.year)}-{str(d.month)}-{str(d.day)}T00:00:00'
...
def GenkWLoad(bld_name, figure_on):
    ...
    for t in new_time:
        ...
        elif t - 900 * 96 in newload_map and t - 900 * 97 in newload_map:
            newload_map[t] = newload_map[t-900] + newload_map[t-900*96] - newload_map[t-900*97]
        elif t - 900 * 192 in newload_map and t - 900 * 193 in newload_map:
    ...
    timestamp = [datetime.datetime.fromtimestamp(t) for t in new_time]
