...
print(f'We have {N} observations with {D} dimensions')
...
#      - Histogram of the grradient-norm over the Tensor
  ...
  for gradient, variable in gradients:
    ...
    h2 = tf.histogram_summary(f"{variable.name}/gradients", grad_values)
    h3 = tf.histogram_summary(f"{variable.name}/gradient_norm",
...
#    print(result[0])
    ...
    for i in range(max_iterations):
      ...
      if i%100 == 1:
        ...
        acc = result[1]
        print(f"Estimated accuracy at iteration {i} of {max_iterations}: {acc}")
        step += 1
      else:
